...
/**
 * An {@link Authorizer} decorator which uses a Guava {@link Cache} to temporarily cache principals'
 * role associations. <p> Cache entries include both inclusion and exclusion of a principal within a
 * given role.
 *
...
public class CachingAuthorizer<P extends Principal> implements Authorizer<P> {
  ...
  /**
   * A cache which maps (principal, role) tuples to boolean authorization states.
   *
   * A cached value of `true` indicates that the key's principal is authorized to assume the given
   * role. False values indicate the principal is not authorized to assume the role.
   *
   * `null` cache values are interpreted as cache misses, and will thus result in read through to
