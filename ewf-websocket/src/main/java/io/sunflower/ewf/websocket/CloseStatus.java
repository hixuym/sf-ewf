...
public final class CloseStatus {
    ...
    /**
     * "1012 indicates that the servicelocator is restarted. A client may reconnect, and if it chooses
     * to do, should reconnect using a randomized delay of 5 - 30s."
     */
    public static final CloseStatus SERVICE_RESTARTED = new CloseStatus(1012);

    /**
     * "1013 indicates that the servicelocator is experiencing overload. A client should only connect
     * to a different IP (when there are multiple for the target) or reconnect to the same IP upon
     * user action."
     */
    ...
    public CloseStatus withReason(String reason) {
        Preconditions.checkArgument(!Strings.isNullOrEmpty(reason), "Reason must not be empty");
