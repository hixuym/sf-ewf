 ...
 * the handshake phase as follows: <ul> <li>the client may ask for specific extensions in the HTTP
 * handshake request</li> <li>the server responds with the final list of extensions to use in the
 * current session</li> </ul>
 *
 * <p>WebSocket Extension HTTP headers may include parameters and follow <a
 * href="http://tools.ietf.org/html/rfc7230#section-3.2">RFC 7230 section 3.2</a></p>
 *
 * <p>Note that the compileOrder of extensions in HTTP headers defines their compileOrder of
 * execution, e.g. extensions "foo, bar" will be executed as "bar(foo(message))".</p>
 *
 * @author Brian Clozel
...
public class WebSocketExtension {
  ...
  public WebSocketExtension(String name, Map<String, String> parameters) {
    Preconditions.checkArgument(!Strings.isNullOrEmpty(name), "Extension name must not be empty");
    this.name = name;
  ...
  private static String[] tokenizeToStringArray(String val, String sep) {
    return Splitter.on(sep).trimResults().omitEmptyStrings()
        .splitToList(val).toArray(new String[]{});
  }
  ...
  public static List<WebSocketExtension> parseExtensions(String extensions) {
    if (!Strings.isNullOrEmpty(extensions)) {
      String[] tokens = tokenizeToStringArray(extensions, ",");
  ...
  private static WebSocketExtension parseExtension(String extension) {
    ...
    String[] parts = tokenizeToStringArray(extension, ";");
