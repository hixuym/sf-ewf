...
import static io.sunflower.gizmo.Result.SC_403_FORBIDDEN;
import static io.sunflower.gizmo.Result.SC_405_METHOD_NOT_ALLOWED;
import static io.sunflower.gizmo.Result.SC_426_UPGRADE_REQUIRED;
...
import io.sunflower.gizmo.websocket.SubProtocolCapable;
import io.sunflower.gizmo.websocket.WebSocketExtension;
import io.sunflower.gizmo.websocket.WebSocketHandler;
import io.sunflower.gizmo.websocket.WebSocketHttpHeaders;
import io.sunflower.gizmo.websocket.handler.WebSocketHandlerDecorator;
import io.sunflower.gizmo.websocket.server.HandshakeFailureException;
import io.sunflower.gizmo.websocket.server.HandshakeHandler;
    ...
    try {
      ...
      if (!"WebSocket".equalsIgnoreCase(context.getHeader(HttpHeaders.UPGRADE))) {
        ...
        return Results.badRequest()
            .renderRaw("Can \"Upgrade\" only to \"WebSocket\".".getBytes(GizmoConstant.UTF_8));
      }
      ...
      if (!context.getHeader(HttpHeaders.CONNECTION).contains("Upgrade") && !context
        ...
        return Results.badRequest()
            .renderRaw("\"Connection\" must be \"upgrade\".".getBytes(GizmoConstant.UTF_8));
      }
