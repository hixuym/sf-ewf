...
package io.sunflower.ewf.bodyparser;
...
import io.sunflower.ewf.params.ResourceMethodInvokerTest.Dep;
import io.sunflower.ewf.params.ResourceMethodInvokerTest.NeedingInjectionParamParser;
...
public class BodyParserEnginePostTest {
  ...
  public void testBodyParserWithValidationErrors() {
    ...
    assertViolation("integerPrimitive", Constants.INT_KEY);
    assertThat(testObject.integerPrimitive, equalTo(0));
    assertViolation("integerObject", Constants.INT_KEY);
    assertNull(testObject.integerObject);
    assertViolation("longPrimitive", Constants.INT_KEY);
    assertThat(testObject.longPrimitive, equalTo(0L));
    assertViolation("longObject", Constants.INT_KEY);
    assertNull(testObject.longObject);
    assertViolation("floatPrimitive", Constants.FLOAT_KEY);
    assertThat(testObject.floatPrimitive, equalTo(0F));
    assertViolation("floatObject", Constants.FLOAT_KEY);
    assertNull(testObject.floatObject);
    assertViolation("doublePrimitive", Constants.FLOAT_KEY);
    assertThat(testObject.doublePrimitive, equalTo(0D));
    assertViolation("doubleObject", Constants.FLOAT_KEY);
    assertNull(testObject.doubleObject);

    assertViolation("date", Constants.DATE_KEY);
    assertNull(testObject.date);
    assertViolation("timestamp", Constants.DATE_KEY);
